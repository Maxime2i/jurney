...
let mediaRecorder; // Déclaration en dehors des fonctions pour qu'elle soit accessible globalement
...
const audioCapture = (timeLimit, muteTab, format, quality, limitRemoved) => {
  chrome.tabCapture.capture({audio: true}, (stream) => { // sets up stream for capture
    ...
    mediaRecorder = new Recorder(source); // Initialisation de mediaRecorder
    ...
    const maxRecordingTime = 5000; // Durée maximale d'enregistrement en millisecondes

    const startMultipleRecordings = () => {
      mediaRecorder.startRecording();
      setTimeout(() => {
        ...
        mediaRecorder.onComplete = (recorder, blob) => {
          ...
          // a.href = url;
          // a.download = 'enregistrement.' + mediaRecorder.encoding; // Nom du fichier
          ...
          formData.append('file', blob, 'enregistrement.' + mediaRecorder.encoding);

          fetch('http://localhost:1500/api/transcribe', {
            method: 'POST',
            body: formData
          })
          ...
          .then(data => {
            ...
            // Envoyer la réponse au popup
            chrome.runtime.sendMessage({ type: "apiResponse", data: data });
          })
          .catch(error => {
            console.error('Erreur lors de l\'envoi à l\'API:', error);
            console.log('Erreur:', error);
            chrome.runtime.sendMessage({ type: "apiResponse", data: error });
          });

          // Démarrer un nouvel enregistrement après un délai
          setTimeout(() => {
            mediaRecorder = new Recorder(source); // Réinitialiser le mediaRecorder
            mediaRecorder.setEncoding(format); // Réinitialiser l'encodage
            mediaRecorder.setOptions({timeLimit: timeLimit / 1000}); // Réinitialiser les options
            startMultipleRecordings(); // Démarrer le nouvel enregistrement
          }, 1000); // Délai avant le prochain enregistrement
        };
      }, maxRecordingTime);
    };

    startMultipleRecordings(); // Démarrer le premier enregistrement
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.currentTab && sessionStorage.getItem(request.currentTab)) {
    ...
    startCapture();
  } else if (request.command === "stopAllRecordings") {
    stopAllRecordings();
  }
});
...
const stopAllRecordings = () => {
  if (mediaRecorder) {
    mediaRecorder.finishRecording(); // Terminer l'enregistrement en cours
    mediaRecorder.onComplete = (recorder, blob) => {
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.style.display = 'none';
    ...
    mediaRecorder = null; // Réinitialiser mediaRecorder
    console.log('Tous les enregistrements ont été arrêtés.');
  } else {
...
document.getElementById('stopAllRecordings').addEventListener('click', stopAllRecordings);
